...
helpers.dbutils.fs.ls("/FileStore/ronald.kayondo/last_connection_time_of_charge_point")
...
def most_recent_message_of_charge_point(input_df: DataFrame) -> DataFrame:
    ...
    window = Window.partitionBy('charge_point_id').orderBy(col('converted_timestamp').desc())

    # input_df = input_df.select('message_id','converted_timestamp','charge_point_id') \
    input_df = input_df.withColumn('rn',row_number().over(window)) \
                .where(col('rn') == 1)
