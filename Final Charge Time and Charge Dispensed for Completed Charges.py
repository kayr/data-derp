...
def join_with_start_transaction_request(
    input_df: DataFrame, join_df: DataFrame
) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.join(join_df, input_df.message_id == join_df.message_id).select(
...
start_transaction_response_df = df.filter(
    (df.action == "StartTransaction") & (df.message_type == 3)
).transform(convert_start_transaction_response_json)

start_transaction_request_df = df.filter(
    (df.action == "StartTransaction") & (df.message_type == 2)
).transform(convert_start_transaction_request_json)

start_transaction_df = start_transaction_response_df.transform(
    join_with_start_transaction_request, start_transaction_request_df
)
...
def join_stop_with_start(input_df: DataFrame, join_df: DataFrame) -> DataFrame:
    input_df =  input_df.alias('input')
    join_df = join_df.alias('join')
    ### YOUR CODE HERE
    return input_df.join(join_df, input_df.new_body.transaction_id == join_df.transaction_id, "left").select(
        col('join.charge_point_id').alias('charge_point_id'),
        input_df.new_body.transaction_id.alias('transaction_id'),
        join_df.meter_start,
...
def convert_start_stop_timestamp_to_timestamp_type(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.withColumn("start_timestamp",to_timestamp('start_timestamp')) \
...
def calculate_total_time_hours(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    sub_fn = input_df.stop_timestamp.cast('long') - input_df.start_timestamp.cast('long')
    return input_df.withColumn('total_time',round(sub_fn/3600.0,2))
    ###
...
def calculate_total_energy(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.withColumn('total_energy',(input_df.meter_stop - input_df.meter_start).cast('double'))
    ###
...
def convert_metervalues_to_json(input_df: DataFrame) -> DataFrame:
    ...
    body_schema = StructType(
        [
            StructField("connector_id", IntegerType(), True),
            StructField("transaction_id", IntegerType(), True),
            StructField("meter_value", meter_value_schema, True),
        ]
    )

    input_df = input_df.withColumn('new_body',from_json('body',body_schema))
    return input_df
    ###


df.filter((df.action == "MeterValues") & (df.message_type == 2)).transform(
    convert_metervalues_to_json
).printSchema()
df.filter((df.action == "MeterValues") & (df.message_type == 2)).transform(
    convert_metervalues_to_json
).show()
...
def reshape_meter_values(input_df: DataFrame) -> DataFrame:
    ...
    ### YOUR CODE HERE
    return (
        input_df.withColumn("transaction_id", input_df.new_body.transaction_id)
        .select(
            "transaction_id",
            explode(input_df.new_body.meter_value).alias("meter_value"),
        )
        .withColumn("timestamp", to_timestamp("meter_value.timestamp"))
        .select(
            "*",
            explode("meter_value.sampled_value").alias("value_obj"),
        )
        .select(
            "timestamp",
            "transaction_id",
            col("value_obj.value").cast("double").alias("value"),
            col("value_obj.phase"),
            col("value_obj.measurand"),
        )
        .where((col("measurand") == "Power.Active.Import") & (col("phase").isNull()))
    )
    ###


result = (
    df.filter((df.action == "MeterValues") & (df.message_type == 2))
    .transform(convert_metervalues_to_json)
    .transform(reshape_meter_values)
)
display(result)
...
from pyspark.sql.functions import when, sum, abs, first, last, lag, rank
from pyspark.sql.window import Window


def calculate_total_parking_time(input_df: DataFrame) -> DataFrame:

    window_by_transaction = Window.partitionBy("transaction_id").orderBy(
        col("timestamp").asc()
    )

    time_window_subgroup = (
        Window.partitionBy("transaction_id", "group_id")
        .orderBy(col("timestamp").asc())
        .rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)
    )

    return (
        input_df.drop("measurand")
        .withColumn("is_chrgn", when(col("value") > 0, 1).otherwise(0))
        .withColumn("above", lag("is_chrgn", 1, 0).over(window_by_transaction))
        .withColumn("newgrp?", when(col("is_chrgn") != col("above"), 1).otherwise(0))
        .withColumn("group_id", sum(col("newgrp?")).over(window_by_transaction))
        .withColumn("first", first("timestamp").over(time_window_subgroup))
        .withColumn("last", last("timestamp").over(time_window_subgroup))
        .withColumn("time-diff", col("last").cast("long") - col("first").cast("long"))
        .where(col("is_chrgn") == 0)
        .groupBy("transaction_id", "group_id")
        .agg(first("time-diff").alias("time-diff"))
        .groupBy("transaction_id")
        .agg((sum("time-diff") / 3600).alias("total_parking_time"))
        .withColumn("total_parking_time", round(col("total_parking_time"), 2))
    )
...
def join_with_target_df(input_df: DataFrame, joined_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.alias('a')
    joined_df = joined_df.alias('b')
    return (input_df.join( joined_df, input_df.transaction_id == joined_df.transaction_id, "left")
    .select('a.*','b.total_parking_time'))
    ###
...
result = (
    ...
    .transform(
        join_stop_with_start,
        df.filter((df.action == "StartTransaction") & (df.message_type == 3))
        .transform(convert_start_transaction_response_json)
        .transform(
            join_with_start_transaction_request,
            df.filter(
