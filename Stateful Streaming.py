...
def unpack_json_from_status_notification_request(input_df: DataFrame) -> DataFrame:
    ...
    return input_df.withColumn('new_body',from_json('body',body_schema))
    ###
...
def select_columns(input_df: DataFrame) -> DataFrame:
    ...
    return input_df.select(
        ...
        input_df.new_body.status.alias("status"),
        to_timestamp(input_df.new_body.timestamp).alias("timestamp"),
    )
    ###


# display(
#     df.transform(read_from_stream)
...
from pyspark.sql.functions import from_json, window, count


def aggregate_window_watermark(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return (
        input_df.withWatermark("timestamp", "10 minute")
        .groupBy(
            "charge_point_id",
            "status",
