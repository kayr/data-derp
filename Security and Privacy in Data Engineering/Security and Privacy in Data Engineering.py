...
def to_decimal(val):
    return float(val)
cleaned_df['lat'] = cleaned_df.lat.map(to_decimal).map(reduce_precision)
cleaned_df['long'] = cleaned_df.long.map(to_decimal).map(reduce_precision)
...
def add_small_time_noise(timestamp):
    d = timedelta(seconds=uniform(1, 60), minutes=uniform(1, 20))
    return timestamp + d
